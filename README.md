...
픽셀" 상자에 배치된다. **높이가 크기이고 폭이 모양이다** — 높이 480 은 저술 축이라
작은 창에서만 줄어들고(그게 UI 가 커 보이게 하는 장치다), 폭은 창의 종횡비를 따라
4:3 에서 16:9 사이를 움직인다. 그 사이의 창은 막대 없이 꽉 차고, 더 좁으면 위아래,
더 넓으면 좌우에 막대가 생긴다. `src/core/frame.js`.
...
경기 화면 콘솔에서 부르는 것 하나: `docs/cone-audit.js` — 조준 콘이 배경 위에서
읽히는지를 화면 픽셀로 잰다. §11 의 침해 불가 항목이고, 절차는 그 파일 머리말에.
