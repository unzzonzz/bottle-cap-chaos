...
export const PALETTE = {
  ...
  bg: {
     ...
     * 내려다보므로 화면을 채우는 건 돔의 아래쪽이고, 거기에 `skyLow` 를 두면
     * 상대 휘도 0.87 짜리 면이 화면 전체를 덮는다 — 블룸 임계값 0.72 를 한참
     * 넘겨서 필드 주변이 하얗게 탄다. 실제로 그렇게 됐다.
     *
     * 그래서 아래쪽은 따로 있고 임계값 아래(0.46)에 머문다. `skyLow` 는 여전히
  ...
  liquid: {
     ...
     *
     * 액면의 밝은 링은 색이 아니라 **정점 색의 배수**로 만든다 — 코어 색에
     * 1.18~1.28 을 곱한다(`bottleGeometry.buildLiquidGeometry`). 그래야 액체 색을
     * 바꿀 때 링이 저절로 따라오고, 두 값이 어긋날 자리가 없다.
