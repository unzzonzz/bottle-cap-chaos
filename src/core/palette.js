...
export const PALETTE = {
  ...
  water: {
    ...
    caustic: '#eaf6fd',
    /**
     * 물 위에 앉는 **모든** 글자의 잉크. 하나뿐이다.
     *
     * ── 왜 흰색이 아닌가 ────────────────────────────────────────────────────
     * 메뉴의 글자는 두 레이어에 나뉘어 산다. 제목은 월드 레이어라 **블룸을 받고**,
     * 내비와 숫자는 UI 레이어라 안 받는다. 순백(`whiteCool`, 선형 0.95)은 블룸
     * 브라이트패스(0.72)를 한참 넘어 제목을 흰 덩어리로 태운다 — 실측으로 그렇게
     * 됐다. 그러니 흰색은 두 레이어가 **같이** 쓸 수 없는 값이다.
     *
     * `bluePale` 은 선형 0.685 로 임계 바로 아래이고, 물 위에서 5.07:1 이 나온다
     * (실측: 글자 없는 물 0.0949). §11 의 4.5 를 넘는다. 두 레이어가 같은 값을
     * 낼 수 있는 유일한 밝기대이고, 물 아래의 흰 것이 파랗게 어두워지는 것은
     * 물리적으로도 맞다.
     *
     * 위계는 색이 아니라 **크기**가 만든다 — 내비의 12px 과 10px. 단일 웨이트
     * 서체에서 굵기를 못 쓰는 것과 같은 규율이다.
     */
    ink: BLUE_PALE,
  },
