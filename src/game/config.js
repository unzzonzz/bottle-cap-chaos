...
export const CONFIG = {
  ...
  cardFx: {
    ...
    // ── 뚜껑 뒤집기 ──────────────────────────────────────────────────────
    /**
     * 컬링에서 뚜껑이 한 바퀴 넘어가는 데 걸리는 시간.
     *
     * ── 이 값이 시뮬레이션에 닿지 않는다는 것이 여기 있는 이유다 ────────────
     * 뒤집기의 **논리**는 즉시다. `Match.flipCap` 이 바디를 그 자리에서 돌리고,
     * 마찰은 그 포즈에서 나오고, 스냅샷은 이미 새 세계의 것이다. 이 숫자가 정하는
     * 것은 그 다음 0.3초 동안 그림이 무엇을 하는가뿐이다.
     *
     * 그래서 `config.curling` 이 아니라 여기다. `curling` 은 `SYNCED_CONFIG_PATHS`
     * 에 있어서 값이 다르면 매칭이 거절되는데, 두 사람이 서로 다른 속도로 같은
     * 연출을 보는 것은 데스싱크가 아니다. 물리에 닿지 않는 값이 온라인 호환성을
     * 깨는 자리에 있으면, 다음에 그 폴더를 만지는 사람은 그것이 시뮬레이션의
     * 일부라고 읽는다.
     *
     * 0.3 은 부록 J3.5 의 0.25~0.35 한가운데다. 횟수 제한이 없으므로 연타를
     * 견뎌야 하는데, 이 길이면 두 번 연속 눌러도 앞의 것이 절반쯤에서 끊기고
     * 새로 시작하는 것이 끊긴 것으로 읽히지 결함으로 읽히지 않는다.
     */
    flipSeconds: 0.3,
    /**
     * 넘어가는 동안 떠오르는 높이. 뚜껑 지름의 배수.
     *
     * "살짝 떠올랐다 내려앉는다". 살짝인 것이 중요하다 — 거의 수직으로 내려다보는
     * 카메라에서 높이는 크기로 읽히므로, 크게 띄우면 뒤집히는 것이 아니라 뚜껑이
     * 잠깐 커졌다 작아지는 것이 된다.
     */
    flipHop: 0.35,
    /**
