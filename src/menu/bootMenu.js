...
export function bootMenu(
  ...
  /**
   * 제목도 **UI 레이어**다. 그리고 내비보다 **위**에 그린다.
   *
   * ── 순서가 세 번 바뀌었고, 지금 것이 마지막 이유다 ──────────────────────
   * 처음엔 월드 레이어였다 — 병보다 뒤에 그려져야 했기 때문이다. 잉크가 순백이
   * 되면서 블룸이 글자를 태워 UI 로 피신시켰다가, 병이 돌아오면서 제목이 병 앞에
   * 오는 문제로 다시 월드로 돌렸고(그때 블룸을 껐다), 병이 없어지면서 그 제약도
   * 없어졌다.
   *
   * 지금 UI 인 이유는 다르다: 제목의 둘째 줄이 뒤를 **가리기** 때문이다.
   * 글자 둘레의 물빛 테가 지나가는 것을 끊는데, 그러려면 제목이 그것들보다
   * 나중에 그려져야 한다. 월드 패스는 UI 패스보다 먼저 끝나므로 월드에 있는 한
   * 내비를 가릴 수 없다.
   *
  ...
  function runTransition(onSwap, { uncover = true } = {}) {
    ...
    transition.begin(
      null,
      {
         ...
         * 뚜껑은 화면 한가운데에서 카메라를 향해 온다.
         *
         * 병이 있던 시절에는 메뉴에서만 뚜껑이 주둥이에서 그 축을 따라 떠났고
         * 다른 화면에서는 허공에서 나왔다. 후자가 문제로 보고돼 갈래를 두었는데,
         * 병이 없어지면서 모든 화면이 같은 화면이 됐다. 갈래도 없다 — 화면을
