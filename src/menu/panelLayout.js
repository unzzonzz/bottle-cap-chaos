...
export function stackRows(box, isTight = (id) => String(id).startsWith('#chips:')) {
  ...
  for (const r of box.rows) {
    ...
    r.y = edge - h / 2;
    /**
     * 다음 줄까지의 간격은 눈금 줄도 **보통 줄만큼** 자리를 차지한 것으로 친다.
     *
     * 눈금의 배치 높이를 선 두께로 둔 것은 **위**로 붙이기 위해서다. 그 값을
     * 아래에도 쓰면 눈금 줄이 자기 몫의 절반(row/2)을 내놓지 않아, 다음 줄의
     * 중심이 그만큼 위로 올라온다 — 실측으로 줄 사이가 42 인데 눈금 뒤만
     * 33.5 였다. 그래픽 눈금과 카메라 추적이 붙어 보이던 것이 이것이다.
     *
     * 눈에 보이는 리듬은 잉크의 중심들 사이의 거리다. 그러니 붙이는 쪽만
     * 선으로 치고, 띄우는 쪽은 줄로 친다.
     */
    edge = r.y - (isChip ? row : h) / 2;
  }
  return box;
}
