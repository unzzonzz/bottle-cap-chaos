...
export function buttonTexture(label, state, { width, height, scale = 1, role = null }) {
   ...
   *
   * 여백은 그림자를 위한 것이었다. 판이 10 픽셀 번지는 그림자를 지고 있었고,
   * 캔버스가 판에 딱 맞으면 그 번짐이 텍스처 가장자리에서 잘려 한쪽만 그림자가
   * 있는 것처럼 보였다.
   *
   * 그림자는 없어졌다(§19 · §24). 여백은 남는다 — 이제 하는 일이 다르다:
   * `roleButton` 의 밑줄이 라벨 아래로 `CTA.underlineGap` 만큼 내려가고, 여백이
   * 없으면 그 선이 텍스처 아래 변에 붙거나 잘린다.
   */
  const pad = 6;
...
export function victoryPlateTexture(text, color, { width, height, scale = 1 }) {
  ...
  /**
   * ── 판이 없다. 스코어와 같은 처리이고, 같은 이유다 ──────────────────────
   * 이 자리에는 판이 두 번 있었다. 처음에는 각진 흰 판 양 끝에 두꺼운 팀 색 막대
   * 두 개 — 막대의 근거는 "몇 픽셀 폭이 되어도 값이 아니라 **색**으로 읽히는 쪽"
   * 이었고, 근처의 다른 판들이 전부 같은 어휘를 쓸 때는 맞았다. 그 다음에는
   * 막대를 뺀 판 하나였다.
   *
   * PHASE 4 의 감사가 그 판도 없앤다. 판이 하는 일은 글자를 읽히게 하는 것인데
   * 이 글자는 `playerInk` 이고 그것이 놓이는 곳은 목재이거나 잔디이며 둘 다
   * 4.5:1 을 넘는다 — 판이 없어도 읽힌다. 판이 실제로 하는 일은 **필드를 가리는
   * 것**이고, 이 순간은 플레이어가 마지막 판면을 보는 순간이다.
   *
   * 그래서 캔버스는 투명하고 글자만 있다. 뒤에서 필드를 죽이는 일은 레터박스가
   * 이미 하고 있으므로, 여기서 한 번 더 할 이유가 없다.
   */
...
export function modalTexture(
   ...
   *
   * 그림자로 깊이를 말하던 시절에는 이것이 가장 깊은 단이었다 — 모달은 뒤의 모든
   * 것을 막고 있으므로. 그림자가 없어진 뒤 그 일을 하는 것은 **베일**이다:
   * `ModalLayer` 가 뒤를 `ui.veil` 로 덮고, 덮인 것이 뒤에 무언가가 있다는 말이다.
   */
